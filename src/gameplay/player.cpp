	...
	public:
		...
		bool translate(const Input & input)
		{
			...
			case Input::State::DOWN:
				...
				if (stateHID.keyMap.keyMap[(unsigned int)input.getButton()])
					return false;

				stateHID.keyMap.keyMap[(unsigned int)input.getButton()] = true;

				switch (input.getButton())
				{
				case Input::Button::KEY_ARROWDOWN:
				{
					stateHID.dirFlags += StateHID::DIR_OUT;
					break;
				}
				case Input::Button::KEY_ARROWUP:
				{
					stateHID.dirFlags += StateHID::DIR_IN;
					break;
				}

				case Input::Button::KEY_ARROWLEFT:
				{
					stateHID.dirFlags += StateHID::DIR_LEFT;
					break;
				}
				case Input::Button::KEY_ARROWRIGHT:
				{
					stateHID.dirFlags += StateHID::DIR_RIGHT;
					break;
				}
			...
			case Input::State::UP:
				...
				stateHID.keyMap.keyMap[(unsigned int)input.getButton()] = false;
