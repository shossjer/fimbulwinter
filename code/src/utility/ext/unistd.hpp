...
namespace ext
{
	...
	inline ssize read_some_nonzero(int fd, void * buffer, usize size)
	{
		assert(size != 0);

	try_again:
		const ssize n = ::read(fd, buffer, size);
		if (n < 0)
		{
			if (errno == EINTR)
				goto try_again;
	...
	// fd - blocking file descriptor
	inline ssize read_some(int fd, void * buffer, usize size)
	{
		return size == 0 ? 0 : read_some_nonzero(fd, buffer, size);
	}

	// fd - blocking file descriptor
	inline usize read_all_nonzero(int fd, void * buffer, usize size)
	{
		...
		do
		{
			...
			const ssize n = read_some_nonzero(fd, ptr, remaining);
			if (n <= 0)
				break;
	...
	// fd - blocking file descriptor
	inline usize read_all(int fd, void * buffer, usize size)
	{
		return size == 0 ? 0 : read_all_nonzero(fd, buffer, size);
	}
