...
namespace utility
{
	...
	namespace detail
	{
		...
		template <typename Iter>
		// static auto is_contiguous_iterator_impl(Iter x, float) -> decltype(is_contiguous_iterator_impl(x.base(), 0)); // does not work so we have to write this other mess instead
		static auto is_contiguous_iterator_impl(Iter x, decltype(x.base(), float())) { return decltype(is_contiguous_iterator_impl(x.base(), 0)){}; }
	}
	template <typename Iter>
